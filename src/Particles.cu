...
int mover_PC(struct particles *part, struct EMfield *field, struct grid *grd, struct parameters *param)
{
    ...
    commonUsage(-1, part, field, grd, param);
    return (0); // exit succcesfully
} // end of the mover

__global__ void particles_gpu_cal(struct particles *part, struct EMfield *field, struct grid *grd, struct parameters *param)
{
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= part->nop)
        return;

    commonUsage(idx, part, field, grd, param);
}
...
int mover_PC_gpu(struct particles *part, struct EMfield *field, struct grid *grd, struct parameters *param)
{
    ...
    cudaMalloc(&GPUPart, sizeof(particles));
    cudaMalloc(&GPUField, sizeof(EMfield));
    cudaMalloc(&GPUGrd, sizeof(grid));
    cudaMalloc(&GPUParam, sizeof(parameters));
    ...
    int blockNum = (part->nop + threadPerBlock - 1) / threadPerBlock;

    // print species and subcycling
    std::cout << "***  GPU MOVER with SUBCYCLYING " << param->n_sub_cycles << " - species " << part->species_ID << " ***" << std::endl;

    //@@ Launch the GPU Kernel here
    particles_gpu_cal<<<blockNum, threadPerBlock>>>(GPUPart, GPUField, GPUGrd, GPUParam);

    //@@ Copy the GPU memory back to the CPU here
    // Copy part struct
    cudaMemcpy(part, GPUPart, sizeof(particles), cudaMemcpyDeviceToHost);
    cudaMemcpy(field, GPUField, sizeof(EMfield), cudaMemcpyDeviceToHost);
    cudaMemcpy(grd, GPUGrd, sizeof(grid), cudaMemcpyDeviceToHost);
    cudaMemcpy(param, GPUParam, sizeof(parameters), cudaMemcpyDeviceToHost);
